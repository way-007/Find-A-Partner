...
onMounted(async () => {
  ...
  const result = await request.get("/team/get", {
    params: {
      id: route.query.id
    }
  })
  if(result?.code === 0 && result?.data) {
    ...
    result.data.expireTime = result.data.expireTime.substring(0,10)
    formDatas.value = result.data
  }
})


// 02. 完成更新之后提交数据到update接口
const onSubmit = async () => {
  ...
  const result = await request.post('/team/update', requestParmas);
  if(result?.code === 0 && result?.data) {
    Toast.success("编辑队伍成功")
    router.push({
      path: '/family',
      replace: true
    })
